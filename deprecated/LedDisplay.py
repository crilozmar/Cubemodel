...
try:
    ...
    class LedDisplay(PyArtist):
        ...
        _SETTING_NORMALISATION = "Normalisation"
        _SETTING_BRIGHTNESS_STATIC = "Maximum brightness"
        _SETTING_MIN_BRIGHTNESS = "Minimum brightness"
        _SETTING_COLOR = "Colormap"
        #_SETTING_INFINITE_DURATION = "Use infinite pulses"
        _SETTING_PULSES_DURATION = "Finite or infinite pulses"
        ...
        def __init__(self):
            ...
            self.addSetting(self._SETTING_NORMALISATION, ChoiceSetting(["Default","Log10","x^(-1/2)","x^(-1/3)","x^(-1/4)","x^(-1/5)","x^(-1/6)"],0))
            self.addSetting(self._SETTING_BRIGHTNESS_STATIC, RangeSetting(0.3, 1.0, 32, 1))
            self.addSetting(self._SETTING_MIN_BRIGHTNESS, RangeSetting(0.0, 0.2, 32, 0.0))
            ...
            self.addSetting(self._SETTING_PULSES_DURATION, ChoiceSetting(["Infinite","Default finite","Bubbles-like finite"],0))
        ...
        def _brightnessCalculator(self,accumulated_charge,normalisation,power,smallest_charge):
            if accumulated_charge > 0:
                if self.setting(self._SETTING_NORMALISATION) == 0:
                    value = (accumulated_charge/normalisation)**power
                    return value
                else:
                    if normalisation == smallest_charge:
                        #This will happen if the "event" has only one pulse. That pulse should shine
                        return 1.
                    else:
                        if self.setting(self._SETTING_NORMALISATION) == 1:
                                myfun =  lambda x : np.log10(x)
                                minval = myfun(smallest_charge) #Since now big negative values will come, one has to determine the lowest value
                        elif self.setting(self._SETTING_NORMALISATION) == 2:
                                myfun =  lambda x : x**(1./2.)
                                minval = 0.
                        elif self.setting(self._SETTING_NORMALISATION) == 3:
                                myfun =  lambda x : x**(1./3.)
                                minval = 0.
                        elif self.setting(self._SETTING_NORMALISATION) == 4:
                                myfun =  lambda x : x**(1./4.)
                                minval = 0.
                        elif self.setting(self._SETTING_NORMALISATION) == 5:
                                myfun =  lambda x : x**(1./5.)
                                minval = 0.
                        elif self.setting(self._SETTING_NORMALISATION) == 6:
                                myfun =  lambda x : x**(1./6.)
                                minval = 0.
                        maxval = myfun(normalisation)
                        value = (myfun(accumulated_charge)-minval)/(maxval-minval) #Apply myfun and normalise in [0,1] interval
                        return value
            else:
                return 0.0
                
        def _brightnessController(self,value):
            maxval = self.setting(self._SETTING_BRIGHTNESS_STATIC)
            minval = self.setting(self._SETTING_MIN_BRIGHTNESS)
            if value >= maxval:
                return maxval
            elif value == 0:
                return minval
                ...
                if minval < maxval:
                    ValueError("ERROR: Min brightness < max brightness!")
        ...
        def _handleOMKeyMapTimed(self, output, geometry, omkey_pulses_map):
            ...
            if self.setting(self._SETTING_PULSES_DURATION) == 0:
                duration = None
            else:
            ...
            if self.setting(self._SETTING_NORMALISATION) == 0:
                ...
                smallest_charge = 0. #Not used in this case. This is only necessary for the log10 case.
                ...
                for led in charges:
                    ...
                    total_charge = 0.
                    for t,q in pulses:
                        if (q != 0) and (q < smallest_charge):
                            smallest_charge = q
                        total_charge += q
                    if total_charge > max_sum_charges2:
                        max_sum_charges2 = total_charge
                normalisation = max_sum_charges2
            ...
            for led in charges:
                ...
                if duration:
                    ...
                    if self.setting(self._SETTING_PULSES_DURATION) == 1:
                        t0s.append(t0)
                    elif self.setting(self._SETTING_PULSES_DURATION) == 2:
                        for pulse in pulses:
                            t0s.append(pulse[0])
                    else:
                        raise ValueError("Pulses duration problem")
                    ...
                    while tail < len(pulses):
                        ...
                        while pulses[head][0]+duration > t and head >= 0:
                            ...
                            head -= 1
                        brightness_value = self._brightnessCalculator(accumulated_charge,normalisation,power,smallest_charge)
                        if (self.setting(self._SETTING_BRIGHTNESS_STATIC)) < 1 or (self.setting(self._SETTING_MIN_BRIGHTNESS)) > 0:
                            brightness_value = self._brightnessController(brightness_value)
                    ...
                    while head < len(pulses):
                        ...
                        tail = head + 1
                        while tail < len(pulses):
                            accumulated_charge += pulses[tail][1]
                            tail += 1
                        brightness_value = self._brightnessCalculator(accumulated_charge,normalisation,power,smallest_charge)
                        if (self.setting(self._SETTING_BRIGHTNESS_STATIC)) < 1 or (self.setting(self._SETTING_MIN_BRIGHTNESS)) > 0:
                            brightness_value = self._brightnessController(brightness_value)
                    ...
                    for t, q in pulses:
                        ...
                        brightness_value = self._brightnessCalculator(accumulated_charge,normalisation,power,smallest_charge)
                        #brightness_value = accumulated_charge**power
                        if (self.setting(self._SETTING_BRIGHTNESS_STATIC)) < 1 or (self.setting(self._SETTING_MIN_BRIGHTNESS)) > 0:
                            brightness_value = self._brightnessController(brightness_value)
                        """
                        print "charge", bigQ
                        print "normalised charge", accumulated_charge
                        print "brightness value", brightness_value
                        
                        if brightness_value > 1:
                            print "val", brightness_value
                            print "charge", accumulated_charge
                            print "normalisation", normalisation
                            print "power", power
                            print "(charge/normalisation)**power", (accumulated_charge)**power
                        """
                        brightness.add(brightness_value, t)
