...
\item Python package \texttt{pyusb} (get it with \texttt{pip} AND \texttt{pip3}). It is important at that the pyusb package is installed via pip and pip3, since steamshovel runs on Python2 and the EEPROM scripts in Python3
\item All the stuff from folders \texttt{muenster\_icecube\_display}
...
this:\\
\texttt{\$ python3 update\_eeprom.py display\_config\_muenster.json -f
}~\\
\texttt{Found device }~\\
...
\begin{itemize}
\item Open a shell with Icetray:\\
\texttt{\$ {Icetray or [}directory of IceTray software{]}/build/ ./env-shell.sh
}~\\
\item You should have seen a welcome message. Now start steamshovel:
\texttt{\$ steamshovel}
